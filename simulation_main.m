...
for n1 = n1m(1):n1m(2)              % governs initial cell number
    for n2 = n2m(1):n2m(2)          % governs initial density
        for n3 = n3m(1):n3m(2)      % governs strength of allee effect (1 = no proliferation)
            for n4 = n4m(1):n4m(2)  % governs bimodality
                ...
                % particle parameters
                Vmin  = 0.001;           % depth of potential
                alpha = 3;                % steepness of potential
                sigma = sqrt(8)/100*ones(1,N);
                if n4 == 2
                    sigma(2:2:end) = sqrt(8)/1000;
                end
                ...
                for i = 1:N
                    %                   create_cell(time,parent,neighbours,location,local_density,birth_evolution)
                ...
                for k = 1:K
                    ...
                    if (lambda0 + lambda1 + omega) > 0
                        ...
                        for i = 1:min(N,N_old)
                            ...
                            if tmp2 < tmp
                                any_bd = any_bd + 1;
                                any_b  = any_b + 1;
                                disp(['cell number ' num2str(i) ' gave birth to cell number ' num2str(newindex) '.'])
                                crand     = 2*pi*rand;
                            ...
                            if tmpD2 < tmpD
                                ...
                                disp(['cell number ' num2str(i) ' died.'])
                            else
                    ...
                    if mod(k,3600) == 0
                        disp(['Hour: ' num2str(k/3600) '. Dataset: ' num2str(dset) '/' num2str(span(n1m)*span(n2m)*span(n3m)*span(n4m)) '.'])
                ...
                filename2 = strcat(c1,c2,c3,'_',c4,c5,'_neighbo','.mat');
                save(strcat('Datasets\',filename),'observed_cells')
                save(strcat('Datasets\',filename2),'observed_neighbours')
